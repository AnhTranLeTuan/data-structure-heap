...
public class MinHeap {
    ...
    public Integer remove(){
        ...
        int smallerChildIndex = indexOfTheSmallerChild(leftNodeChildIndex(itemIndex), rightNodeChildIndex(itemIndex));

        if (smallerChildIndex == -1)
            return value;

        while (collection.get(itemIndex) > collection.get(smallerChildIndex)){
            bubbleDown(itemIndex, smallerChildIndex);

            itemIndex = smallerChildIndex;

            smallerChildIndex = indexOfTheSmallerChild(leftNodeChildIndex(itemIndex), rightNodeChildIndex(itemIndex));
    ...
    private int indexOfTheSmallerChild(int leftNodeChildIndex, int rightNodeChildIndex){
